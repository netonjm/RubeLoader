	...
	public class b2dJson
	{
		...
		public  bool ReadFromFile(string filename, CCNode node)
		{
			...
			try
			{
				...
				return j2CCPhysicsWorld(worldValue, node);
			}
		...
		public bool j2CCPhysicsWorld(JObject worldValue, CCNode node)
		{
			...
			if (null != bodyValues)
			{
				...
				for (i = 0; i < numBodyValues; i++)
				{
					...
					CCPhysicsBody body = j2CCPhysicsBody(space, bodyValue);
		...
		CCPhysicsBody j2CCPhysicsShape(JObject fixtureValue, int type)
		{
			if (null == fixtureValue)
				return null;

			//List<CCPhysicsShape> allShapes;
			...
			else if (null != fixtureValue["polygon"])
			{
				...
				body = CCPhysicsBody.CreatePolygon(points, numVertices, 0.0f);
